...
#         (user_data/strategies/futures/FOttStrategy.py). Educational use only.
#
# EDITS vs. the fetched original (marked "EDIT" below):
#   1. `from numpy.lib import math` removed - unused, and fails on NumPy >= 2.0.
#   2. `can_short = True` added - the fetched file emits enter_short signals but did not
#      set this flag, so freqtrade would silently ignore all shorts.
#   3. The GitHub page flattened indentation inside ott(); loop/return placement was
#      reconstructed (loops propagate longstop/shortstop and trend/dir; OTT is computed
#      once after). Diff against the repo file before relying on it.
import logging
...
class FOttStrategy(IStrategy):
    # Buy params, Sell params, ROI, Stoploss and Trailing Stop are values generated by
    # 'freqtrade hyperopt --strategy Supertrend --hyperopt-loss ShortTradeDurHyperOptLoss
    #  --timerange=20210101- --timeframe=1h --spaces all'
    # It's encouraged you find the values that better suit your needs and risk management.
    INTERFACE_VERSION: int = 3

    can_short = True  # EDIT: added (see header)
