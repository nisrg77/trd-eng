"""
brain/models/xgb_model.py — [M2] XGBoost (Non-Linear ML)
...
_WEIGHT_FILE = os.path.join(config.MODEL_WEIGHTS_DIR, "xgb_model.joblib")
...
class XGBModel:
    """
    XGBoost regressor wrapper with walk-forward retraining.
    ...
    def _save(self) -> None:
        ...
        log.debug("XGB weights saved → %s", _WEIGHT_FILE)
    ...
    def _load(self) -> None:
        if os.path.exists(_WEIGHT_FILE):
            ...
            log.info("XGB weights loaded from %s", _WEIGHT_FILE)
    ...
    def train(self, X: np.ndarray, close: pd.Series) -> None:
        ...
        if len(Xt) < 10:
            log.warning("XGB: insufficient training data (%d rows)", len(Xt))
            return

        self.model.fit(Xt, y, verbose=False)
        self.is_trained = True
        ...
        log.info("XGB trained on %d samples | importances: %s", len(Xt), self.feature_importances)
        self._save()
    ...
    def predict(self, x_latest: np.ndarray) -> float:
        if not self.is_trained:
            log.warning("XGB not trained — returning 0.0")
            return 0.0
